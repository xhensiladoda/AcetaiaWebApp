	...
	private String check(String idBarile, String tipo, String quantita) {
		...
		if(quantita!="") {
			...
			try {
				...
				error_q = "La quantità deve essere un valore intero.";
			}
			if(q!=0 && idBarile!=null){
				// check: il livello di riempimento non deve superare quello massimo del barile
				bar = Integer.parseInt(idBarile);
				bar-=1; // devo accedere a una lista e usare bar come indice
				max = Integer.parseInt(barile[2].getData(bar));//il maxlivello è la terza colonna
				lev = Integer.parseInt(barile[3].getData(bar));//il livello è la quarta colonna
				int diff = max - lev;
				if ((diff < q) && (q!=0) ) {
					error_lev = "Per questo barile la quantità non deve essere superiore a "+diff+".";
				}
			}
		}
